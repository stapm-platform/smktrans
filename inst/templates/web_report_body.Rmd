...
# Point directly to the centralized outputs folder
data_dir <- file.path(proj_root, "transition_probability_estimates", "outputs")

if (!dir.exists(data_dir)) {
  warning("Could not find the centralized data directory: ", data_dir)
}

# 5. HELPER TO LOAD FILES
# -----------------------
# Map internal prefixes to the sheet names used in the Excel workbook.
sheet_lookup <- c(init = "Initiation", quit = "Quitting", relapse = "Relapse")

# Locate the most recent Excel workbook for this country.
# This is the fallback data source: the *_forecast_data_*.rds intermediates
# are git-ignored (see .gitignore), so on the pkgdown build server and on any
# fresh clone only the committed .xlsx outputs are available.
find_excel <- function() {
  files <- list.files(
    data_dir,
    pattern = paste0("^SmokeStateTransProbs_", config$country, "_.*\\.xlsx$"),
    full.names = TRUE
  )
  if (length(files) == 0) return(NULL)
  sort(files, decreasing = TRUE)[1]  # most recent by date in filename
}

get_res <- function(prefix) {

  # --- Fast path: use the pre-computed .rds if it exists locally ---
  ...
  if (file.exists(f_path)) {
    ...
    # Unpack the $data dataframe if it's a list
    if (!inherits(obj, "data.frame") && "data" %in% names(obj)) {
      dt <- obj$data
    } else {
      dt <- obj
    }
  ...
  # --- Fallback: read the equivalent sheet from the committed Excel workbook ---
  sheet <- sheet_lookup[[prefix]]
  xl    <- find_excel()

  if (!is.null(sheet) && !is.null(xl) && file.exists(xl)) {
    dt <- tryCatch(
      readxl::read_excel(xl, sheet = sheet),
...
# Dynamically find the actual Excel file generated for this country
excel_files <- list.files(data_dir, pattern = paste0("SmokeStateTransProbs_", config$country, "_.*\\.xlsx$"))

if (length(excel_files) > 0) {
  # Grab the most recent one if multiple exist
  excel_name <- sort(excel_files, decreasing = TRUE)[1]
} else {
  # Fallback
