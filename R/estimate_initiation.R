...
#' @param survey_data Data.table. Cleaned survey data.
#'
#' @export
estimate_initiation <- function(config, survey_data) {
  ...
  model_choice <- if(config$country == "Scotland") "Scotland" else "model2"
  
  ever_smoke_data <- smktrans::ever_smoke(
    data = survey_data,
    time_horizon = 2100, # Max horizon
    num_bins = 7,
    model = model_choice, 
  ...
  # E. Forecast (The Critical Missing Step)
  # -------------------------------------------------------------------------
  message("   > Forecasting Initiation Trends...")
  
  # Note: Assumes quit_forecast() is loaded in your environment
